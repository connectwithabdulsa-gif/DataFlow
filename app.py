...
if not ADSENSE_PUBLISHER_ID and ADSENSE_CLIENT_ID.startswith("ca-"):
    ADSENSE_PUBLISHER_ID = ADSENSE_CLIENT_ID.removeprefix("ca-")
ADSENSE_HOME_SLOT_ID = os.environ.get("ADSENSE_HOME_SLOT_ID", "").strip()
ADSENSE_ENABLED_PAGES = {"home", "about", "terms", "contact"}
ADS_TXT_TEMPLATE = "google.com, {publisher_id}, DIRECT, f08c47fec0942fa0\n"
...
def ads_txt():
    if not ADSENSE_PUBLISHER_ID:
        return jsonify({"error": "AdSense publisher ID not configured"}), 503
    return app.response_class(
        ADS_TXT_TEMPLATE.format(publisher_id=ADSENSE_PUBLISHER_ID),
        mimetype="text/plain",
    )
